...
const ChannelChat = () => {
  ...
  const channelId = (params["entity-id"] as string).slice(2);

  const { user: currentUser } = useCurrentUser();

  const getChannel = useGetChannel(workspaceId, channelId);
  const { messages: channelMessages, isLoading: isLoadingMessages } =
  ...
  if (getChannel.isLoading || isLoadingMessages || !currentUser) {
    return (
  ...
  const channel = transformChannel(getChannel);
  const messages = transformMessages(channelMessages || []);
