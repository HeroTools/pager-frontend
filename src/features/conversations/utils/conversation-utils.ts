...
export const findExistingConversation = (
  ...
  return conversations.find((conversation) => {
    ...
    const otherMemberIds =
      conversation.other_members?.map((member) => member.workspace_member?.id).filter(Boolean) ||
      [];
    ...
    const allMembersMatch =
      lengthsMatch && sortedTargetOtherIds.every((id, index) => id === sortedOtherIds[index]);
