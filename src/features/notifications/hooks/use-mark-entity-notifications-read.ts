...
export const useMarkEntityNotificationsRead = () => {
  ...
  const markEntityNotificationsRead = useCallback(
    async (workspaceId: string, entityId: string, entityType: 'channel' | 'conversation') => {
      try {
        ...
        if (previousUnreadCount) {
          queryClient.setQueryData<{ unread_count: number }>(
            notificationKeys.unreadCount(workspaceId),
            {
              unread_count: Math.max(0, previousUnreadCount.unread_count - countToDecrease),
            },
          );
        }
          ...
          (old) => {
            ...
            const newPages = old.pages.map((page) => ({
              ...
              notifications: page.notifications.filter(
                (notification) => !notificationIdsToMarkAsRead.includes(notification.id),
              ),
              unread_count: Math.max(0, page.unread_count - countToDecrease),
            }));
        ...
        try {
          notificationsApi.markNotificationAsRead(workspaceId, notificationIdsToMarkAsRead);
